...
//This section is used for the Netskope API key
    //It is highly reccomended to use environment variables, but wanted to provide an easy way to recreate the demo if needed.
        //Below are the commands to use to set environment variables on a system. 
        //If you use the provider block, DO NOT commit this file to a public or unsecured repository. 
...
//This block will create the private applicaiton within Netskope, and assign the app to the publisher, based on the output from the
    //AWS Module
resource "netskope_privateapps" "PrivateApp" {
    app_name = "aws private app"
    host     = "172.0.16.54"
